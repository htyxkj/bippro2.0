...
let dateAdd = function (fps) {
  ...
  const hou = fps[4];
  const min = fps[5];
  return moment().add(5, 'd').add(mon, 'M').add(year, 'y').add(hou, 'h').add(min, 'm');
}
...
let nextQuote4 = function (cs0, x0, x1, ch) {
  for (x0++; x0 < x1; x0++) {
    var c0 = cs0[x0];
    if (c0 == ch)
      return x0;
...
let nextBarcket4 = function (cs0, x0, x1, chL) {
  var chR = chL == '[' ? ']' : chL == '(' ? ')' : '}';
  var ct = 0;
  for (x0++; x0 < x1; x0++) {
    var c0 = cs0[x0];
...
let nextBarcket = function (buf, cs0, x0, x1, chL) {
  var c0, chR = chL == '(' ? ')' : (chL == '[' ? ']' : '}');
  buf += (chL);
  x0++;
...
let nextQuote = function (buf, cs0, x0, x1, ch) {
  buf += ch;
  for (x0++; x0 < x1; x0++) {
    var c0 = cs0[x0];
    if (c0 == ch) {
...
// 两个数值做运算
let calcTwoNumber = function (o0, o1, cfh, cpnt) {
  if (cfh == '+' || cfh == '-' || cfh == '*' || cfh == '/') {
    if (cfh == '+') {
...
let calcTwoValue = function (o0, o1, ysf) {
  if (ysf == '+' || ysf == '|') {
    if (o0 == null)
      return o1;
    if (o1 == null)
...
let calcTwoObject = function (o0, o1, ysf, cpnt) {
  return this.calcTwoNumber(o0, o1, ysf, cpnt);
};
let calcTwoItem = function (o0, o1, cfh) {
...
let calcItems = function (ovs, fhs, idx) {
  var x0 = idx - 2,
    x1 = idx - 1,
 ...
 * @param {数据对象} data 
 */
let formatVarMacro = function (sql, cells, data) {
  return this.formatVarMacro4(sql, cells, data, true);
}

let formatVarMacro4 = function (str, cells, data, btrue) {
  // let x0 = str.indexOf('{^'); 中文字符先不考虑
  return this.formatVar(str, cells, data);
}

let formatVar = function (sql, cells, data) {
...
let formatVar2 = function (sql, cells, data, xf) {
  let xt = null, st0 = null, sk;
  while (xf > -1) {
...
let nexttoken = function (sv0, bnext) {
  let x1 = sv0.length - 1;
  let cs0 = sv0.split('');
  if (bnext) {
    if (cs0[0] === ')')
...
let proc_SqlVar = function (sv, cells, data) {
  ...
  if (sk !== null && sk.length > 0) {
    ...
    if (bDT) {
      ...
      return sv === null || sv.length < 1 ? null : this.sqlDate(sf, chinn === 'T', sk, sv);
    }
    if (chinn === 'C')
      sv = this.sqlString(sv);
 ...
 * @param {*} bL 
 */
let getCondItem = function (sv, bL) {
  let t0 = sv !== null ? sv.length - 1 : -1, x0;
  if (t0 < 0)
    return null;
  let sFH = ",;+~";
  if (bL) {
 ...
 */
let toBlurCond = function (fname, type, ocd, blvl) {
  let sc0 = ocd === null || typeof (ocd) === 'string' ? ocd : this.objToString(ocd, type);
  if (sc0 === null || sc0.length < 1)
    return null;
  let chinn = this.sqlTypeToChar(type);
  let t0, x0, ic0 = 0;
  let sr0 = '', st0, s0, s1, slk;
  let cor = this.findIndex4(sc0, ';', 0, true, false) > -1 ? ';' : ',';
  let cand = this.findIndex4(sc0, '~', 0, true, false) > -1 ? '~' : '+';
  let band = false, b0, b1, bdt = chinn === 'D' || chinn === 'T';
  while (sc0 != null) {
    ...
    if (x0 < 0)
      st0 = this.toBlurConda(fname, blvl ? this.trimLevel(st0) + "%" : st0, chinn);
    else {
      t0 = st0.charAt(0) === '(' ? 1 : 0;
      if (x0 > t0) {
        ...
        slk = t0 > 0 ? ">" : ">=";
        if (bdt)
          s0 = this.sqlDate(fname, chinn === 'T', slk, s0);
        else {
          if (chinn === 'C')
            s0 = this.sqlString(this.filterESC(s0, null));
          s0 = fname + slk + s0;
        }
      } else
        s0 = null;
      ...
      if (x0 <= t0) {
        ...
        slk = b1 ? "<=" : "<";
        if (bdt)
          s1 = this.sqlDate(fname, chinn === 'T', slk, s1);
        else {
          if (chinn === 'C')
            s1 = blvl && b1 ? "'" + s1 + "zz'" : this.sqlString(this.filterESC(s1, null));//;-注意分级指定前面值时包括子级
          s1 = fname + slk + s1;
        }
        if (s0 == null)
          st0 = s1;
...
let toBlurConda = function (fname, sc0, ct) {
  let t0 = sc0.length;
  let c0 = sc0.charAt(0);
  let b0 = c0 === '!';
  if (b0) {
    if (t0 === 1)
      return fname + " is not null";
    sc0 = sc0.substring(1);
  } else if (c0 === '\\')
    sc0 = sc0.substring(1);
  else if (c0 === '=' && t0 === 1)
    return fname + " is null";
  let sk = b0 ? "<>" : "=";
  if (ct === 'D' || ct === 'T')
...
let sqlDate = function (fName, btime, slk, sc) {
  if (fName === null)
    fName = "";
  if (fName.length < 1 || fName.charAt(0) !== '@') {
    sc = this.checkDate(sc, false);
    return "[%" + (btime ? "T" : "D") + fName + (slk == null ? "" : slk) + sc + "]";
  }
  fName = fName.substring(1);//;--@
  let x0 = sc.indexOf('-'), x1;
  let s0 = "1";
  if (x0 > 0) {
    x0++;
    x1 = sc.indexOf('-', x0);
  ...
  x0 = fName.indexOf(',');
  if (x0 < 0)
    return fName + slk + sc + (s0.length < 2 ? "0" + s0 : s0);
  x1 = fName.indexOf('.');
  return "(" + fName.substring(0, x0) + slk + sc + " and " + (x1 > 0 ? fName.substring(0, x1 + 1) : "") + fName.substring(x0 + 1) + slk + s0 + ")";
}

let checkDate = function (s0, ymd) {
...
let dateArrToString = function (xs, x0, x1, bdiv) {
  let sb = '', iv = 0;
  if (bdiv) {
    let sdiv = "--- :: ";
    while (x0 < x1) {
...
let dateToAry = function (sd) {
  let cc = sd === null ? 0 : sd.length;
  if (cc < 1)
    return null;
  let cs0 = sd.split(''), c0 = cs0[0].charCodeAt(0);
  if ((c0 < '0'.charCodeAt(0) || c0 > '9'.charCodeAt(0)) && c0 !== ';'.charCodeAt(0) && c0 !== ','.charCodeAt(0))
    return null;
...
let isAutoFit = function (o0) {
  if (o0) {
    let cc = o0.length;
    if (cc < 1)
      return false;
    for (let i = 0; i < cc; i++) {
      if ("%_?~+;,".indexOf(s0.charAt(i)) >= 0)
        return false;
    }
    return true;
  }
...
let objToString = function (value, sqltype) {
  if (value) {
    if (sqltype === 91) {
...
let dateToString = function (ov, bdiv, fmt) {
  if (ov) {
    if (fmt === 4) {
    ...
    if (fmt === 6 || fmt === 2) {
      return moment(ov).format(bdiv ? "YYYY-MM-DD HH:mm:ss" : "YYYYMMDD HH:mm:ss");
    }
    if (fmt === 3) {
...
let getCellIndex = function (cells, sv) {
...
let toVarYMD = function (sv, ch0, cc) {
  if (ch0 === 'Y' && (cc === 1 || (sv.charAt(1) === 'M' && (cc === 2 || (cc === 3 && sv.charAt(2) === 'D')))))
    return cc === 1 ? 4 : (cc === 2 ? 0 : 1);
  if (cc === 1 && (ch0 === 'y' || ch0 === 'm'))
    return 0x100 + (ch0 == 'y' ? 4 : 5);//--年月独立。
  if (cc === 2 && ch0 === 'y' && sv.charAt(1) === 'm')//--年月独立。
    return 0x100 + 0;
  return -1;
}

let sqlTypeToChar = function (type) {
  if (type === 12 || type === 1 || type == -9 || type == -16)
    return 'C';
  if (type === 3 || type === 6 || type === 8 || type === 2)
    return 'N';
  if (type === 5 || type === 4 || type === -5)
    return 'I';
  return type === 91 ? 'D' : (type === 93 || type === 92 ? 'T' : 'C');
}

let toF_K_V = function (sv0) {
  let c0 = '=', cs0 = sv0.split('');
  let t0 = cs0.length, x0 = 0;
  for (x0 = 0; x0 < t0; x0++) {
    c0 = cs0[x0];
    if (c0 === '=' || c0 === '>' || c0 === '<' || c0 === ' ')
      break;
...
let toCorpSQLVar = function (sf, ico) {
  let c0 = sf.charAt(0);
  let cf = sf.length;
  if ((c0 == '_' || c0 == '*') && (cf < 2 || this.findIndex4(sf, "=<>", 0, true) < 0)) {
    if (ico < 1)
      return "";
...
let findIndex4 = function (str, sdiv, xfr, bsp) {
  let c0 = null;
  let cs0 = typeof (str) === 'string' ? str.split('') : str;
  let cc = cs0 === null ? 0 : cs0.length;
  for (; xfr < cc; xfr++) {
    c0 = cs0[xfr];
    if ((bsp && c0 <= ' ') || sdiv.indexOf(c0) >= 0)
      return xfr;
  }
  return -1;
}
 ...
 * 查找单一分隔符。
 */
let findIndex = function (str, cch, xfr, quote, barcket) {
 ...
 * 查找多分隔符。
 */
let findIndex2 = function (str, cch, xfr, quote, barcket, exdiv) {
  let c0 = null;
  let cs0 = typeof (str) === 'string' ? str.split('') : str;
  let cc = cs0 === null ? 0 : cs0.length;
  let bex = exdiv !== null && exdiv.length > 0, bc0 = cch === '\0';
  if (bc0)
    cch = ';';
  for (; xfr < cc; xfr++) {
    c0 = cs0[xfr];
    if (c0 === cch || (bc0 && cch === ',') || (bex && exdiv.indexOf(c0) >= 0))
      return xfr;
    if (c0 === '\\')
      xfr++;
    else if (quote && (c0 === '\'' || c0 === '"'))
...
let sqlQuery = async function(sql,type){
  var  posParams = {
...
let doAPi = async function(posParams){
