                            ...
                            <md-layout md-row>
                                <md-layout>
                                    ...
                                    <md-button class="md-primary md-raised md-icon-button" @click="clear">
                                        ...
                                        <md-tooltip md-direction="right">清除飞行数据</md-tooltip>
                                    </md-button>
                                    <!-- <md-button class="md-primary md-raised md-icon-button" @click="fullScreen">
                                        <md-tooltip md-direction="right">全屏</md-tooltip>
                                    </md-button>
                                    <md-button class="md-primary md-raised md-icon-button" @click="exitFullScreen">                                       
                                        <md-tooltip md-direction="right">退出全屏</md-tooltip>
                                    </md-button> -->
...
export default {
    ...
    methods: {
        select() {
            var _this = this
            this.$mapDialog.open({ diaTitle: '轨迹回放参数选择',redio:true, getOpt(planeno, taskno, start, end,redio,isShowZYQ) { _this.getTim(planeno, taskno, start, end,redio,isShowZYQ) } })
        },
        clear(){ 
        ...
        getTim(planeno, taskno, start, end,redio,isShowZYQ) {
            this.i=0;
            this.loading = 1
            this.points=[];
            //清楚飞机覆盖物 
            ...
            this.creatLin(planeno, taskno, start, end,redio)
            //获取异常信息参考值
        ...
        //划线
        creatLin(planeno, taskno, start, end,redio){
            ...
            .then(res=>{
                ...
                this.LuShu(this.map,this.points,'#339933',true)
                ...
                var icon = new BMap.Icon(require('@/img/map/airfence/end.png'), new BMap.Size(30, 30), { imageOffset: new BMap.Size(1, 1) }) 
                var tkname
                if(`${d.taskName}`!=""){
                    tkname = `任务名称:${d.taskName}\n`;
                    this.taskName= "　　"+`${d.taskName}`+"　　"
                }
                var title = tkname+`用户标识:${d.planeId}\n`+`当前时间:`+d.time;
                var marker = new BMap.Marker(d.pt, {icon:icon, title: title })
                this.map.addOverlay(marker)

                var d0 = this.points[0]
                var icon0 = new BMap.Icon(require('@/img/map/airfence/start.png'), new BMap.Size(30, 30), { imageOffset: new BMap.Size(1, 1) }) 
                var title0 = tkname+`用户标识:${d0.planeId}\n`+`当前时间:`+d0.time;
                var marker0 = new BMap.Marker(d0.pt, {icon:icon0, title: title0 })
